...
export default function AdminPanel() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const result = await createAdminUser(emailInput, passwordInput, {
        nombre: nombreInput,
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedUser) {
        ...
        toast.success('Cambios guardados con éxito', { id: loadingToast });
  ...
  const openEditModal = (user: any) => {
    ...
    setFechaNacimientoInput(user.fecha_nacimiento || '');
    setRolInput(user.rol || 'usuario');
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white p-6 md:p-8 relative">
      ...
      <div className="relative z-10 max-w-7xl mx-auto space-y-8">
       ...
       {isCreateModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 text-white">
          <form onSubmit={handleCreate} className="bg-neutral-900 border border-neutral-800 rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            <h3 className="text-xl font-bold text-white mb-2">Añadir Nuevo Usuario</h3>
            
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs text-neutral-400 mb-1 block">Nombre</label>
                <input required type="text" value={nombreInput} onChange={e => setNombreInput(e.target.value)}
              ...
              <div>
                ...
                <input required type="text" value={apellidoInput} onChange={e => setApellidoInput(e.target.value)}
                  className="w-full bg-black border border-neutral-700 rounded-lg px-4 py-2 text-white outline-none focus:border-orange-500" />
              </div>
            </div>

            <div>
              <label className="text-xs text-neutral-400 mb-1 block">Fecha de Nacimiento</label>
              <input type="date" value={fechaNacimientoInput} onChange={e => setFechaNacimientoInput(e.target.value)}
                className="w-full bg-black border border-neutral-700 rounded-lg px-4 py-2 text-white outline-none focus:border-orange-500" />
            </div>

            <div>
              <label className="text-xs text-neutral-400 mb-1 block">Rol del Sistema</label>
              <select 
                value={rolInput} 
                onChange={e => setRolInput(e.target.value)}
                className="w-full bg-black border border-neutral-700 rounded-lg px-4 py-2 text-white outline-none focus:border-orange-500 appearance-none"
              >
            ...
            <div>
              <label className="text-xs text-neutral-400 mb-1 block">Correo Electrónico</label>
              <input required type="email" value={emailInput} onChange={e => setEmailInput(e.target.value)}
                className="w-full bg-black border border-neutral-700 rounded-lg px-4 py-2 text-white outline-none focus:border-orange-500" />
            </div>
            <div>
              <label className="text-xs text-neutral-400 mb-1 block">Contraseña</label>
            ...
            <div className="flex justify-end gap-3 mt-4 pt-4 border-t border-neutral-800">
              ...
              <button type="submit" className="bg-orange-600 hover:bg-orange-500 text-white px-5 py-2 rounded-lg font-bold transition shadow-lg shadow-orange-600/20">
                Crear Usuario
              </button>
            </div>
          </form>
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 text-white">
          <form onSubmit={handleEdit} className="bg-neutral-900 border border-neutral-800 rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            <h3 className="text-xl font-bold text-white mb-2">Editar Usuario</h3>
            ...
            <div>
              <label className="text-xs text-neutral-400 mb-1 block">Nueva Contraseña (Opcional)</label>
              <input type="password" placeholder="Sólo para cambiar" value={passwordInput} onChange={e => setPasswordInput(e.target.value)}
                className="w-full bg-black border border-neutral-700 rounded-lg px-4 py-2 text-white outline-none focus:border-orange-500" />
            </div>
